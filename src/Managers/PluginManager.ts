...
export default class PluginManager {
	...
	public static async queuePluginForEnable(
		plugin: PgPlugin
	): Promise<boolean> {
	...
	public static async loadNewPlugins() {
		if (PluginManager.getKnownPluginIds() === null) {
			PluginManager.setKnownPluginIds(
			...
			installedPlugins?.forEach((id) => {
				if (!knownPlugins?.has(id)) {
					Manager.getInstance().groupsMap.forEach((g) => {
						if (g.autoAdd) {
							const plugin =
								PluginManager.getInstalledPluginFromId(id);
							if (plugin) {
								g.addPlugin(plugin);
							}
						}
	...
	private static checkPluginEnabledFromPluginGroups(
		plugin: PgPlugin
	): boolean {
	...
	private static enablePlugin(plugin: PgPlugin): Promise<boolean> {
		return Manager.getInstance().obsidianPluginsObject.enablePlugin(
			plugin.id
		);
	}

	private static disablePlugin(plugin: PgPlugin) {
		return Manager.getInstance().obsidianPluginsObject.disablePlugin(
			plugin.id
		);
	}
}
